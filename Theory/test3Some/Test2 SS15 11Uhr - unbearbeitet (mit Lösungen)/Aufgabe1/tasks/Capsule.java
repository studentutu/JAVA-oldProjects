...
public class Capsule {
    ...
    */
    ...
    public void move(double deltaLongitude, double deltaLatitude) {
        ...
        if(this.longitude+deltaLongitude >= 180){
            double zwischen=this.longitude+ deltaLongitude;
            zwischen=(zwischen)%180;

            this.longitude=-180+zwischen;
        ...
        if(this.longitude+deltaLongitude<=-180){
            double zwischen=this.longitude+ deltaLongitude;
            zwischen=(zwischen)%180;
            zwischen=Math.abs(zwischen);

            this.longitude=180-zwischen;

        }
        if((this.latitude + deltaLatitude>=90 )){
            if(this.latitude>=0){
                this.latitude=-180 + this.latitude;
            }else{ this.latitude=180 + this.latitude;}
            double zwischen=this.latitude+deltaLatitude;
            zwischen=zwischen%90;
            this.latitude=-90+zwischen;
        }
    ...
    public String location() {
         // TODO: Implementation is your task
        return String.Format("Long: %1.4f , Lat: %2.2d ",this.longitude,this.latitude);
    }
